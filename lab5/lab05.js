...
function showWindowHref() {
    const index = url.value.indexOf("name=");
    if (index === -1)
        url_result.value = "Not found";
    else {
        const index1 =url.value.indexOf("&", index);
        if (index1 === -1)
            url_result.value = url.value.substring(index + 5);
        else
            url_result.value = url.value.substring(index + 5, index1);
    }
}
...
function arrSameStr() {
    let Most, MostCount = 0, tMostCount, many = most.value.split("");
    for (let i = 0; i < many.length; i++) {
        tMostCount = 0;
        for (let j = 0; j < many.length; j++)
            if (many[j] === many[i])
                tMostCount++;
        if (tMostCount > MostCount) {
            Most = many[i];
            MostCount = tMostCount;
        }
        result.value = "The most character is:" + Most + " times:" + MostCount;
    }
}
